...
export function buildHeavyTaskSystemPromptPolicy(
  ...
  return [
    ...
    '- Follow this thin work loop until the task is done or a real cap is reached: minimal_public_inspection -> acceptance_dag -> inventory -> runnable_artifact -> public_check -> repair_or_continue -> node_self_checks -> semantic_self_check -> finish_or_continue.',
    '- Before implementation, editing, broad command loops, or claiming a path is correct, call acceptance_dag_submit. You may do only minimal public instruction/workspace inspection first so the DAG is grounded in visible files.',
    '- The acceptance DAG must use stable node ids and explicit dependencies. Include required nodes for requirement extraction, final deliverable contract, implementation, public positive check, at least one negative/invariance check when practical, and final audit.',
    '- Every DAG node must list concrete acceptanceCriteria derived only from visible public task/workspace evidence. Keep acceptance criteria testable by public commands, artifact inspection, or deterministic reasoning over visible files.',
    '- As work advances, refresh acceptance_dag_submit. Before finalization every required completed DAG node must include a node selfCheck with status pass plus concrete public commandEvidence or artifactEvidence; blocked/cancelled nodes need public evidence explaining why they are nonblocking.',
    ...
    '- Self-check loop ownership: you own implementation, repair, and model_self_check evidence. The runtime self-check controller owns adversarial tester scheduling. Do not call agent_spawn for adversarial checking, and do not submit adversarial_check_plan_submit or adversarial_check_execution_submit; those are runtime-owned ledger events in heavy-task autonomous runs.',
    '- Before final self_check_submit, call self_check_plan_submit. Declare final artifacts, the self-check scratch root, workspace guard checked paths, expected added or generated paths, and only public model-owned checks you actually intend to run.',
    '- Runtime adversarial checkpoint: after you submit a pass self_check_submit, the controller will automatically run a test-duty adversarial_check child agent. On the first accepted candidate it generates a public task-derived suite, materializes and executes it; on repair rounds it reruns that same suite only. If the adversarial execution fails or is inconclusive, the controller will return a repair prompt with public failure evidence and repair recommendations. Repair the deliverable, rerun your public checks, and submit a fresh self_check_submit.',
    ...
    '- Use self_check_submit after your own public checks have run or been inspected. Submit public, task-derived semantic self-check evidence from visible tests, builds, sample commands, or artifact inspections. Include public command/artifact evidence only; the runtime will add adversarial tester evidence after your submission.',
    ...
    '- Heavy-task finalization stays in a self-check repair loop while the latest self_check_plan_submit or self_check_submit is missing, failing, inconclusive, inconsistent, lacks sandbox/workspace guard evidence, misses visible required artifacts, or fails runtime-owned adversarial checks. Treat each gate message as a diagnostic: compare it against your accepted plan, public evidence, and adversarial tester feedback, then repair and resubmit self_check_plan_submit and self_check_submit as needed. The loop is bounded by runtime step, tool-call, wall-clock, and harness timeouts rather than a fixed repair-attempt budget.',
